    ...
    <div class="container">
        <form method="POST" action="{{route('produto.store')}}">
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.010" name="preco" class="form-control" id="input-preco" placeholder="Digite o preço">
            </div>
            <div class="form-group">
                <label for="select-tipo">Tipo</label>
                <select id="select-tipo" class="form-control" name="tipo">
                    @foreach ($resourceArray as $resource)
                        <option value="<?= $resource['id'] ?>"><?php echo $resource['descricao'] ?></option>
                    @endforeach
                </select>
            </div>
